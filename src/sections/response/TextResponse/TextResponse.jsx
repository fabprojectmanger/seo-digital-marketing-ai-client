...
const TextResponse = () => {
  ...
  async function getChatGPTResponse() {
    ...
    if (googleResponse) {
      ...
      response = await SeoChatAI.initiateChat(prompt, setAiResponse, {
  ...
  return (
    <div
      className={`seo__text-response-cont h-full flex flex-col ${
        !aiResponse && !restartRequired.current ? "justify-center" : ""
      } ${restartRequired.current ? " justify-center" : ""}`}
    >
      ...
      {restartRequired.current ? (
        <div className="seo__text-response-restart h-full -mt-24">
          <Image
            src={RestartRobot.src}
            alt="Restarting robot..."
            width={RestartRobot.width}
            height={300}
          />
          <H4 className={`text-center !text-2xl uppercase text-dark-100`}>
        ...
        aiResponse && (
          <StreamResponse
            paragraph={aiResponse}
            className={classEnabled ? " g-t " : " "}
          />
        )
      )}
    </div>
