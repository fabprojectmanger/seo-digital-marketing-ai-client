...
const AuditSection = ({ title, audits, accordionKey, setAccordionActive, accordionActive, renderDetails, tab, label }) => (
  tab === label && 
    audits.length > 0 && (
      <Wrapper className="accordion-items my-5 space-y-2">
        ...
        {audits.map((item, i) => (
          <Wrapper key={i} className="accordion-item bg-white p-4 rounded-md">
            <Wrapper>
              <div
                onClick={() => setAccordionActive((prev) => ({
                  ...prev,
              ...
              >
                <Wrapper className="gap-2 flex items-center">
                  ...
                  <div dangerouslySetInnerHTML={{ __html: TextToHTML(item?.title) }} />
                </Wrapper>
                <IconDownArrow
                  size={20}
                  className={`${
            ...
            >
              <div className="py-4" dangerouslySetInnerHTML={{ __html: TextToHTML(item?.description) }} />
              {renderDetails(item)}
            </Wrapper>
          </Wrapper>
        ))}
